...
test("account service updates runtime settings for an account", () => {
  ...
  assert.equal(
    next.envs.default.accounts.personal.runtime.openaiBaseUrl,
...
test("account service rejects selecting an unknown account", () => {
  ...
  assert.throws(
    () =>
      service.selectAccount(createSampleState(), {
        envName: "default",
        accountName: "missing",
        target: "cli",
    ...
    (error: unknown) => {
      assert.equal((error as AccountServiceError).code, "ACCOUNT_NOT_FOUND");
      return true;
    },
  );
});
