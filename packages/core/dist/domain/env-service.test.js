...
test("env service rejects removing the reserved default env", () => {
    const service = createEnvService();
    assert.throws(() => service.removeEnv(createSampleState(), { envName: "default" }), (error) => {
        assert.equal(error.code, "RESERVED_ENV");
        return true;
    });
});
...
test("env service rejects renaming the reserved default env", () => {
    const service = createEnvService();
    assert.throws(() => service.updateEnv(createSampleState(), {
        envName: "default",
