...
test("renderHomeScreen prints logo, menu, and nav hint", () => {
    const screen = renderHomeScreen(0, "Update available", "Mismatch: current launcher PowerShell is not ready; target cmd is ready | Suggestion: ready target available: cmd");
...
test("runHomeLoop renders plain output in non-interactive mode", async () => {
    ...
    }, "", "Mismatch: current launcher bash is not ready; target zsh is ready");
