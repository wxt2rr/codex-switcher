...
test("readUsageProxyState falls back to Clash Verge mixed-port on macOS", async () => {
    ...
    try {
        await writeFileRecursive(join(root, "Library", "Application Support", "io.github.clash-verge-rev.clash-verge-rev", "clash-verge.yaml"), "mixed-port: 7899\n");
        const state = await readUsageProxyState(join(root, ".codex-switcher"), { HOME: root }, "darwin");
...
test("readUsageProxyState reads Clash Verge app config when generated config is absent", async () => {
    ...
    try {
        await writeFileRecursive(join(root, "Library", "Application Support", "io.github.clash-verge-rev.clash-verge-rev", "verge.yaml"), "verge_mixed_port: 7899\n");
        const state = await readUsageProxyState(join(root, ".codex-switcher"), { HOME: root }, "darwin");
        assert.deepEqual(state, {
            source: "auto-system",
            value: "http://127.0.0.1:7899",
