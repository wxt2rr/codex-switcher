...
const DEEPSEEK_V4_FLASH: ModelCatalogEntry = {
...
const DEEPSEEK_MODEL_PRESETS = new Map<string, ModelCatalogEntry>([
...
export function resolveProviderModelPreset(input: {
  ...
  if (!model || !isDeepSeekOfficialBaseUrl(input.baseUrl)) return undefined;
  const entry = DEEPSEEK_MODEL_PRESETS.get(model);
  if (!entry) return undefined;
  return { providerId: "deepseek", catalogPath: "models.json", entries: [entry] };
}
...
export function resolveProviderDefaultPreset(baseUrl?: string): ProviderModelPresetMatch | undefined {
  ...
  return { providerId: "deepseek", catalogPath: "models.json", entries: [DEEPSEEK_V4_FLASH] };
}
