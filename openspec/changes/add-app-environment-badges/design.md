...
macOS uses a bundled N-API module loaded directly into the Electron main process to request Accessibility trust and discover matching Dock accessibility elements; Electron owns the click-through overlays. Keeping AX calls in the authorized app process avoids a separate-helper TCC identity mismatch. The native module observes Dock element move/resize/layout notifications and coalesces refreshes, so app focus, Space changes, and ordinary window activity do not reposition badges; only Dock geometry changes do. Panels are scoped to the Space in which they are created, are stationary and nonactivating, and are recreated after an active-Space change; they are explicitly excluded from fullscreen Spaces. Coordinate conversion is performed against the display containing each Dock element, rather than the active screen. Because Dock AX elements retain stale frames in fullscreen Spaces, discovery also checks whether a layer-zero application window occupies the complete target display and refreshes on active-Space changes. Windows uses a bundled PowerShell/.NET interop adapter to attach a taskbar overlay to resolved HWNDs. The manager treats missing or incompatible native resources as unsupported/partial rather than crashing.
...
- [Accessibility permission can be denied or revoked] → Never persist a successful enable before trust and re-evaluate status on activation.
...
- [Native helper missing or quarantined] → Fail closed, preserve Codex launch behavior, and expose a partial/unsupported status.
