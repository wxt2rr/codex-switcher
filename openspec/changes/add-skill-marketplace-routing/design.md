...
Provider paths are editable because vendors may change them and WorkBuddy installations may differ. New providers can be added through registry data plus a detector without changing skill lifecycle semantics.
...
The Skills page follows existing query-based soft navigation (`?view=skills`) and adds a sidebar item. Its first horizontal tab is Marketplace, followed by `Codex · <environment>` tabs and one tab for every supported provider global directory. A global sync drawer shows every non-Codex provider, its target path, enabled state, and Codex source-environment selector. Provider tabs distinguish unmanaged local skills from managed links and show their Codex source.

Marketplace installation presents one multi-select target list containing every Codex environment and every supported global provider. Selecting a Codex environment installs the canonical skill into that environment. Selecting a global provider also requires one Codex source environment; the renderer installs the canonical skill there when needed, then enables or updates the provider binding so its managed projection is reconciled. Selecting a provider therefore opts that provider into the complete skill set of its chosen source environment, not a one-off unmanaged copy of only the current skill. The currently selected Codex tab is the default target when installation starts from an environment view.
